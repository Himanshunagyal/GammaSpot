...
//     background-image: linear-gradient(rgba(200,241,53,0.025) 1px,transparent 1px), linear-gradient(90deg,rgba(200,241,53,0.025) 1px,transparent 1px);
...
//   .back-btn { background:none; border:1px solid var(--border); color:var(--text-muted); cursor:pointer; font-family:var(--font-mono); font-size:11px; padding:7px 12px; border-radius:4px; letter-spacing:0.05em; transition:all 0.15s; white-space:nowrap; }
...
//   .status-pill.error     { background:var(--red-dim); border-color:rgba(255,77,106,0.3); color:var(--red); }
...
//   .ctrl-btn.stop:hover { background:rgba(255,77,106,0.2); }
...
//   .prov-opt { padding:7px 12px; cursor:pointer; transition:all 0.15s; color:var(--text-muted); border:none; background:none; font-family:var(--font-mono); font-size:clamp(10px,1.5vw,11px); letter-spacing:0.04em; }
//   .prov-opt.active { background:var(--accent-dim); color:var(--accent); }

//   /* Connection banner */
//   .conn-banner { display:flex; align-items:center; justify-content:center; gap:10px; padding:10px; font-family:var(--font-mono); font-size:12px; border-bottom:1px solid var(--border); }
//   .conn-banner.ok     { background:rgba(200,241,53,0.05); color:var(--accent); }
//   .conn-banner.warn   { background:rgba(255,181,71,0.05); color:var(--yellow); }
//   .conn-banner.err    { background:rgba(255,77,106,0.05); color:var(--red); }
...
//   .alert-card.alert-watch { border-color:rgba(255,181,71,0.2); }
...
//   /* Empty / loading state for scanner */
...
//   .scanner-empty-icon { font-size:32px; opacity:0.2; }
...
//     .hero-title { font-size:40px; } .hero-sub { font-size:26px; }
//     .stat-box { flex:1 1 100%; border-right:none; }
//   }
// `;
...
//           <div className="m-card-sub">{inst.sub || "Index · NSE"}</div>
//         </div>
...
//         <div>
//           <div className="m-stat-label">RSI</div>
//           <div className={`m-stat-val rsi-val ${inst.rsi > 60 ? "high" : inst.rsi > 55 ? "mid" : "low"}`}>
//             {inst.rsi ? inst.rsi.toFixed(1) : "—"}
//           </div>
//         </div>
//         <div>
//           <div className="m-stat-label">Volume</div>
//           <div className="m-stat-val" style={{ color: inst.volume_ratio > 1.5 ? "var(--accent)" : inst.volume_ratio > 1 ? "var(--yellow)" : "var(--text-muted)" }}>
//             {inst.volume_ratio ? inst.volume_ratio.toFixed(1) : "—"}x
//           </div>
//         </div>
//       </div>
...
//           📌 BUY {inst.atm_strike} CE · ₹{inst.entry_premium} → ₹{inst.target}
//         </div>
//       )}
//     </div>
//   );
...
//           Real-time multi-index scanner that detects your exact options setup
//           the moment it appears — and alerts you instantly.
//         </p>
...
//           { n: "03", t: "Instant Alerts",    d: "Strike, entry, target and SL delivered to Telegram the moment the setup fires." },
//         ].map(f => (
...
//         <span>GammaSpot v1.0 — Dev Build</span>
//         <span>Groq · Kite Connect · NSE</span>
//       </div>
//     </div>
//   );
// }
...
//       console.log("[WS] Connected to GammaSpot backend");
//     };
...
//           if (msg.instruments && msg.instruments.length > 0) {
...
//           setScanCount(msg.scan_count || 0);
//           setAlertCount(msg.alert_count || 0);
//           setLastScan(msg.scanned_at);

//           // Extract FIRE alerts and add to log
//           const fired = (msg.instruments || []).filter(i => i.signal === "FIRE");
//           fired.forEach(inst => {
//             const alertId = `${inst.symbol}-${msg.scanned_at}`;
//             if (!alertIds.current.has(alertId)) {
//               alertIds.current.add(alertId);
//               const newAlert = {
//                 id:     alertId,
//                 type:   "fire",
//                 name:   inst.display_name || formatName(inst.symbol),
//                 time:   msg.scanned_at || new Date().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }),
//                 action: `BUY ${inst.atm_strike} CE`,
//                 details: {
//                   "Entry":      `₹${inst.entry_premium}`,
//                   "Target":     `₹${inst.target} (+30%)`,
//                   "Stop Loss":  `₹${inst.stop} (-25%)`,
//                   "RSI":        String(inst.rsi),
//                   "Volume":     `${inst.volume_ratio}x avg`,
...
//     setProvider(p);
//     sendCommand("set_provider", { provider: p });
//   };

//   // ── Connection status display ────────────────────────
//   const connBanner = {
//     connected:  { cls: "ok",   text: "● Connected to backend — ready" },
//     connecting: { cls: "warn", text: "◌ Connecting to backend at localhost:8000..." },
//     error:      { cls: "err",  text: "✗ Cannot reach backend — make sure uvicorn is running on port 8000" },
//   }[wsStatus];
...
//             {["groq", "claude"].map(p => (
//               <button key={p}
//                 className={`prov-opt ${provider === p ? "active" : ""}`}
...
//           </div>
//           {!running
//             ? <button className="ctrl-btn start"
...
//       {/* Connection banner */}
//       <div className={`conn-banner ${connBanner.cls}`}>{connBanner.text}</div>

//       {/* Scan bar */}
//       {running
//         ? <div className="scan-bar-wrap"><div className="scan-bar" /></div>
//         : <div style={{ height: 2 }} />
//       }
...
//           { label: "Interval",     value: "60s",                      sub: provider,          cls: "" },
//         ].map(s => (
...
//             {lastScan && <div className="last-scan">Last scan: {lastScan}</div>}
//           </div>
...
//                   : "Start the bot to begin scanning instruments."}
//               </div>
//             </div>
//           ) : (
//             <>
...
//                       const sig = getSignal(inst);
//                       const name = inst.display_name || formatName(inst.symbol);
//                       return (
//                         <tr key={inst.symbol} className={`scanner-row ${sig === "FIRE" ? "active" : ""}`}>
//                           <td>
//                             <div className="instrument-name">{name}</div>
//                             <div className="instrument-sub">₹{inst.spot?.toLocaleString("en-IN") || "—"}</div>
//                           </td>
//                           <td>
...
//                           </td>
//                           <td>
//                             <span className={`rsi-val ${inst.rsi > 60 ? "high" : inst.rsi > 55 ? "mid" : "low"}`}>
//                               {inst.rsi?.toFixed(1) || "—"}
//                             </span>
//                           </td>
//                           <td>
//                             <span style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: inst.volume_ratio > 1.5 ? "var(--accent)" : inst.volume_ratio > 1 ? "var(--yellow)" : "var(--text-muted)" }}>
//                               {inst.volume_ratio?.toFixed(1) || "—"}x
//                             </span>
//                           </td>
//                           <td>
//                             <span className={`signal-badge ${sig === "FIRE" ? "fire" : sig === "WATCH" ? "watch" : "cold"}`}>
...
//                 {instruments.map(inst => <MobileCard key={inst.symbol} inst={inst} />)}
//               </div>
//             </>
//           )}
//         </div>
...
//               <div>No alerts yet.<br />Start the bot to begin scanning.</div>
//             </div>
//           ) : (
//             alerts.map(a => (
//               <div key={a.id} className={`alert-card ${a.type === "fire" ? "alert-fire" : "alert-watch"}`}>
